...
def _node_label(node_type, config):
    """Build a display label for a node based on its current config."""
    label = f"**{node_type}**"
    if node_type == "Inject":
        ...
        act = config.get("action", "set")
        prop = config.get("property", "payload")
        label += f"\n\n`{act} {prop}`"
    elif node_type == "HTTP Request":
        method = config.get("method", "GET")
        url = config.get("url", "")[:25]
        label += f"\n\n`{method} {url}`"
...
def _sync_node_labels():
    """Update every node's visible content to match its current config."""
...
def _make_node(node_name, x=100, y=100):
    """Create a StreamlitFlowNode + default config for a given node type name."""
    ...
    return flow_node, node_id, config
            ...
            for prop_name, prop_info in NODE_CATALOG[node_type]["properties"].items():
                ...
                if prop_info["type"] == "text":
                    ...
                    config[prop_name] = st.number_input(
                        prop_info["label"],
                        value=float(config.get(prop_name, prop_info["default"])),
                        key=key,
                    )
...
with tool_cols[3]:
    ...
    if saved_flows:
        selected_flow = st.selectbox(
            "Load saved flow",
            options=[""] + saved_flows,
            format_func=lambda x: "-- Load a saved flow --" if x == "" else x.replace(".json", "").replace("_", " ").title(),
            key="flow_picker",
            label_visibility="collapsed",
        )
        if selected_flow and selected_flow != st.session_state.get("_loaded_flow"):
            path = os.path.join(FLOWS_DIR, selected_flow)
            st.session_state.flow_state, st.session_state.node_configs = (
                _load_flow_from_file(path)
            )
            st.session_state.debug_output = []
            st.session_state.execution_log = []
...
# ── sync node labels with current config before rendering ──────────
...
if run_clicked:
    ...
    for n in st.session_state.flow_state.nodes:
        nid = n.id
        cfg = st.session_state.node_configs.get(nid, {})
        nodes_data.append({"id": nid, "node_type": cfg.get("_type", "Unknown")})
...
if uploaded is not None:
    try:
        # Write to a temp file so we can reuse _load_flow_from_file
        import tempfile
...
with out_col1:
    ...
    if st.session_state.debug_output:
        for i, msg in enumerate(st.session_state.debug_output):
